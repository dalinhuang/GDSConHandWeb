...
import com.winjune.wips.manager.common.db.HibernateUtil;
...
public abstract class AbstractRepositoryHibernate<T> {
	...
	@SuppressWarnings("unchecked")
	protected List<Object> findByCriteria(String selectedColumn, Criterion... criterion)
			throws RepositoryException {
		try {
			Criteria criteria = getSession().createCriteria(
					getPersistentClass());
			...
			criteria.setProjection(Projections.property(selectedColumn));
	...
	protected Integer count() throws RepositoryException {
		try {
			...
			Number count = (Number) c.setProjection(Projections.rowCount())
					.uniqueResult();
